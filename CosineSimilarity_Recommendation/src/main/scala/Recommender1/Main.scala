...
object  Main extends App {
  ...
  Model.personalNormalizedRatings = normlizedPersonalRatings

  val model = Model.trainModel()
  ...
  val result = Model.predict(userId, model)
  ...
  recommendationList.select("movieId", "title", "genres").collect().foreach(println)
